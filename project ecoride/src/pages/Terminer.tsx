...
export function Terminer() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (carpoolingData.statut !== 'en_cours') {
          setError('Ce voyage n\'est pas en cours');
          return;
        }
  ...
  const handleEndTrip = async () => {
    try {
      ...
      // Create validation documents for each passenger
      const validationPromises = passengers.map((passenger) => {
        return setDoc(doc(collection(db, 'validations')), {
          chauffeur_id: carpooling.driver_id,
      ...
      // Wait for all validation documents to be created
      await Promise.all(validationPromises);

      // Préparer les données du covoiturage
